    ...
    beforeEach(() => {
       ...
       cy.visit('/');
        ...
        it('Fluxos e operações Funcionalidades do Usuario', () =>{                 
            ...
            cy.log ('13.Verificação dos critério de criação com todos digitos numéricos')
            ...
            cy.get(elementos.camponovasenha).eq(1).type(Cypress.env('senhanumeric'))
            cy.contains('Confirmação da nova senha:')
            cy.get(elementos.campoconfirmanovasenha).eq(2).type(Cypress.env('senhanumeric'))           
