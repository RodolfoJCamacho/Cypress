...
describe('Acesso ao Sistema SEG-ID', () => {
  ...
  it('Fluxos e operações Funcionalidades do Usuario', () => {
    cy.log('10.Verificação dos Critérios criação senha menos 8 caracteres');
    ...
    cy.get(elementos.camponovasenha).eq(1).type(Cypress.env('senhacriterio'));

    cy.contains('Confirmação da nova senha:');
    cy.get(elementos.campoconfirmanovasenha).eq(2).type(Cypress.env('senhacriterio'));
