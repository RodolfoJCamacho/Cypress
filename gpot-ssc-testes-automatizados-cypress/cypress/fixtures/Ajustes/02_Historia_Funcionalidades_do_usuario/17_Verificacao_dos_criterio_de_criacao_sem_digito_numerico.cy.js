    ...
    beforeEach(() => {
       ...
       cy.visit('/');
        ...
        it('Fluxos e operações Funcionalidades do Usuario', () =>{                 
            ...
            cy.log ('17.Verificacao dos critério de criacao sem digito numerico')
             ...
             cy.get(elementos.camponovasenha).eq(1).type(Cypress.env('senhasemnumero')) 
             cy.contains('Confirmação da nova senha:')
             cy.get(elementos.campoconfirmanovasenha).eq(2).type(Cypress.env('senhasemnumero')) 
             cy.get(elementos.salvarButton).click()
             cy.wait(1000)
             cy.contains('Senha atual:')
             cy.get(elementos.camposenhatual).first().type(Cypress.env('senhasemnumero'))             
             cy.contains('Nova senha:')
