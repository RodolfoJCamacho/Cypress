...
describe('Fluxo de exceção | Solicitação criação de Usuário', () => {
  ...
  it('', () => {
    ...
    cy.log('15.Solicitação criação usuário Com telefone inválido e telefone em branco')
...
cy.get(elementos.campoInput)
  ...
  .type(Cypress.env('aplicacao'));
                ...
                cy.contains('Telefone:')
                    .siblings()
                    .filter(elementos.telefoneInput).type(Cypress.env('telefoneerror'))  
    ...
    cy.log('16.Solicitação criação usuário Com RG inválido validar mensagem de erro RG Inválido')
    ...
    cy.contains('Identidade (RG):')
      .siblings()
      .filter(elementos.rgInput).type(Cypress.env('rginvalido'))
