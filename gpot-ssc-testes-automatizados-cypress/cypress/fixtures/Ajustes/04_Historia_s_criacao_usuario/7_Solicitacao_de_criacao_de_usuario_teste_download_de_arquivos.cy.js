...
describe('', () => {
    ...
    it('', () => {
        ...
        cy.log('7.Solicitação de criação de usuário teste download de arquivos')
...
cy.get(elementos.campoInput)
  ...
  .type(Cypress.env('aplicacao'));
        ...
        cy.get(elementos.razaosocial).click()
        cy.wait(1000)


        cy.contains('Nome Social:')
            .siblings()
            .filter(elementos.nomeInput).type(Cypress.env('nomesocial'))

        
        const nome = fakerPT_BR.person.fullName()
        cy.contains('Nome:')
            .siblings()
            .filter('[class="z-textbox"]').type(nome)
        ...
        const cpf = br.cpf()
                ...
                cy.get(elementos.download).click()
