...
describe('Fluxo de exceção | Solicitação criação de Usuário', () => {
  ...
  it('', () => {
    ...
    cy.log('19.Solicitação criação usuário Sem incluir permissão')
    cy.wait(2000)
...
cy.get(elementos.campoInput)
  ...
  .type(Cypress.env('aplicacao'));
    ...
    cy.log('20.Solicitação criação usuário unidade papel em branco')
   ...
   cy.get(elementos.menuIcon).click()
    ...
    cy.contains('Unidade:').click().last().click()
    cy.wait(2000)
    cy.get(elementos.ultimoBotaoTema).last().click()
    cy.get('[class="z-icon-times"]').click()
