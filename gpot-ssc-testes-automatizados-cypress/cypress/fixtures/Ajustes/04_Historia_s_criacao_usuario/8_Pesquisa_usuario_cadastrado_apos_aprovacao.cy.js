...
describe('', () => {
    ...
    it('', () => {
        ...
        cy.log('8.Pesquisa usuário cadastrado após aprovação')
...
cy.get(elementos.campoInput)
 ...
 cy.get(elementos.botaoEsquerda)
  .filter((_, el) => el.textContent.trim() === 'USUÁRIO')
  .should('exist')
  .click();
            ...
            cy.get(elementos.cpfInput).eq(0).type(cpf)
             cy.wait(2000)
            cy.get(elementos.btnPesquisar).click()
            cy.contains(Cypress.env('status')).click()
