...
describe('Fluxo de exceção | Solicitação criação de Usuário', () => {
  ...
  it('', () => {
    ...
    cy.log('17.Solicitação criação usuário Com E-mail secundário e-mail gestor inválidos')
    cy.wait(2000)
...
cy.get(elementos.campoInput)
  ...
  .type(Cypress.env('aplicacao'));
                ...
                cy.contains('E-mail do gestor:')
                    .siblings()
                    .filter(elementos.emailGestorInput).type(Cypress.env('emailgestorerrado'))
                ...
                cy.get(elementos.btnPesquisar).click()
                 cy.wait(2000)
                cy.contains(Cypress.env('situacao')).click()
                cy.wait(1000)
    ...
    cy.log('18.Solicitação criação usuário Com celular invalido')
    ...
    cy.get(elementos.criacaodesoslitacoes).eq(2).click();
          ...
          cy.contains(Cypress.env('criacaodeusuario')).click();
               ...
               const cpf1 = br.cpf()
                ...
                cy.contains('Celular:')
                    .siblings()
                    .filter(elementos.celularInput).type(Cypress.env('celularinvalido'))
